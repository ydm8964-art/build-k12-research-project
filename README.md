...
可先复制`references/teacher-profile.example.json`填写教师基本情况，再运行`project_workflow.py start`生成候选题；确认候选ID后运行`select`和`initialize`。系统会建立标准文件夹、26项生命周期材料登记、注意事项、交付索引、工作簿草稿和可迭代任务队列。Agent需继续按任务队列制作、QA、登记并刷新材料；真实数据、照片、签章和官方意见仍按阶段补入，不能由初始化器伪造。
