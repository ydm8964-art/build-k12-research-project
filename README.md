...
- 提供基本信息→5—8项选题→题目确认→主清单→26项材料任务队列的一体化工作流；
- 完整保留校情、教材、班级、人数、问题证据、已有做法和资源，防止从选题到申报材料的信息断链；
- 提供 7 个通用 DOCX 结构母版、1 个含 9 张工作表的 XLSX 数据母版、26项生命周期初始化器及整套审计脚本；
- M00—M25逐项绑定固定版式合同，明确各级字体字号、首行/悬挂缩进、行距、段前段后、页边距及Word表格硬值，并自动套版、自动拦截漂移；
- 标题/正文采用双层审计：既核对字体字号等硬值，也独立识别语义层级，拦截标题套正文、正文套标题、封面信息缩进错误和结构角色缺失；
- 每个新课题强制重新检索当年官方政策和模板，保存项目级快照与哈希；检索超过7天或附件更新时阻止最终交付；
...
- 最终通过预检后自动生成包含整套目录、政策快照、预检报告和SHA-256清单的ZIP，避免散件漏交。
...
可先复制`references/teacher-profile.example.json`填写教师基本情况，再运行`project_workflow.py start`生成候选题；确认候选ID后运行`select`和`initialize`。系统会建立标准文件夹、26项生命周期材料登记、注意事项、交付索引、工作簿草稿和可迭代任务队列。队列同时列出可执行、真实性/模板阻断和等待前置材料三种状态。Agent需继续按任务队列制作、QA、登记并刷新材料及控制文件；真实数据、照片、签章和官方意见仍按阶段补入，不能由初始化器伪造。
