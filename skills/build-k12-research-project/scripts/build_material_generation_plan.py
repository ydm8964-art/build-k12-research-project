...
ROLE_CONTRACTS = {
    "attention-items": ("汇总真实性状态、阻断项、照片任务、原始材料、时间冲突、签章和增强建议", ("governance", "timeline", "evidence", "subject_coverage", "materials")),
    "application": ("写清依据、问题证据、研究目标、内容、方法、创新、团队基础、计划和承诺成果", ("project", "research_questions", "logic_mappings", "timeline", "commitments")),
    "anonymous-form": ("从申请书提炼匿名评审内容并清除身份线索，不改变研究承诺", ("project", "research_questions", "logic_mappings", "commitments")),
    "opening": ("把申请承诺操作化为指标、任务、样本、工具、时间、分工、风险和质量控制", ("logic_mappings", "samples", "instruments", "timeline", "interventions")),
    "consent-form": ("形成适龄告知、数据使用、照片授权、退出机制和联系方式留空区", ("project", "evidence", "governance")),
    "blank-instrument": ("设计与研究问题和学科表现对应的调查或诊断工具，并提供使用说明", ("research_questions", "subject_coverage", "instruments")),
    ...
    "data-workbook": ("从主清单填充项目说明、变量编码、证据、照片和材料进度，保留真实数据空区", ("project", "samples", "instruments", "evidence", "materials")),
    ...
    "intervention-plan": ("把诊断问题映射到核心策略、实施课次、载体、评价、证据和偏离记录", ("logic_mappings", "interventions", "subject_coverage", "timeline")),
    "lesson-plan": ("按学科目标—任务—成果—评价设计可实施课例并登记版本和安全要求", ("interventions", "subject_coverage", "evaluation_weights")),
...
def build(manifest: dict) -> dict:
    ...
    for item in materials:
        ...
        template_blocked = item["format_profile"] == "official-exact" and not item.get("reference_template")
        jobs.append({
            ...
            "execution_state": "blocked-template" if template_blocked else "waiting-dependency",
    ...
    terminal = {"ready", "verified", "submitted", "archived"}
    dependency_available = {"draft", "pending-data", "pending-photo", "pending-signature", *terminal}
    next_jobs = []
    blocked_jobs = []
    for job in jobs:
        if by_id[job["material_id"]]["status"] in terminal:
            continue
    ...
    return {
        "schema_version": "1.0", "snapshot_id": manifest["generation_contract"].get("snapshot_id"),
        ...
        "next_jobs": next_jobs, "blocked_jobs": blocked_jobs, "jobs": jobs,
        "agent_loop": [
            "只处理依赖已满足的next_jobs", "从source_manifest_fields读取唯一事实，不在单份文档内另造事实",
            "用reference_template或对应通用母版制作文件", "完成内容、格式、渲染、隐私及适用的数据/照片QA",
            "用register_material_file.py登记文件和状态", "重新生成本计划，继续下一批，直到所有可完成任务处理完毕",
        ],
    }
